        ...
        <form action="{{ route('update_user', $user['id']) }}" method="POST" class="border-0 w-50">
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <input value="{{ $user['fullname'] }}" required type="text" class="form-control mb-3"
                        name="fullname">
                    <label for="afm" class="form-label">ΑΦΜ</label>
                    <input value="{{ $user['afm'] }}" required minlength="9" maxlength="9" type="number"
                        class="form-control mb-3" name="afm">
                </div>
                <div class="col-6">
                    <label for="username" class="form-label">Username</label>
                    <input value="{{ $user['username'] }}" required type="text" class="form-control mb-3"
                        name="username">
                    <label for="email" class="form-label">Email</label>
                    <input value="{{ $user['email'] }}" required type="email" class="form-control mb-3"
                        name="email">
                </div>
                <div class="col-12">
                    <label for="phone" class="form-label">Τηλέφωνο</label>
                    <input value="{{ $user['telephone'] }}" required type="text" class="form-control mb-3"
                        name="phone">
                </div>
            </div>
